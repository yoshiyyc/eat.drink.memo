...
export default function AdminPage() {
    ...
    try {
      ...
      for (const seed of DRINKS_SEED) {
        ...
        for (const drinkName of seed.drinks) {
          ...
          await addDoc(collection(db, 'drinks'), { shopId: shop.id, name: drinkName, isSeasonal: false });
          added++;
        }
      }
  ...
  if (!isAdmin) {
    return <div className="p-8 text-sm text-muted">沒有權限</div>;
  }

  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="text-[22px] font-bold mb-6">管理後台</h1>

      <div className="border border-border p-6 mb-4">
        <h2 className="text-[15px] font-semibold mb-2">Seed 飲料資料</h2>
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-2 text-sm bg-text text-bg disabled:opacity-50"
        >
          {loading ? '新增中...' : '新增飲料資料'}
        </button>
        {status && (
          <pre className="mt-4 text-sm text-muted bg-border-light p-3 whitespace-pre-wrap">
            {status}
          </pre>
        )}
      </div>
